...
def normalize_qos_intent(qos: Mapping[str, Any]) -> NormalizedQoSIntent:
    ...
    if policy != _SUPPORTED_POLICY:
        raise QoSIntentError(f"QoS v0.1 supports only policy={_SUPPORTED_POLICY}")

    explicit = [field for field in _EXPLICIT_FACTS if field in qos]
    if not explicit:
        return NormalizedQoSIntent(attributes={"enabled": True, "policy": policy})
    ...
    for index, raw in enumerate(raw_classes):
        ...
        priority = _positive_int(raw.get("priority"), f"qos.classes[{index}].priority", maximum=8)
