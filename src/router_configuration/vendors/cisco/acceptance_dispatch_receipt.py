...
request against an exact source ref. It is not live device evidence and cannot
complete any Cisco acceptance stage.
"""
...
def verify_dispatch_receipt(
    ...
    *,
    ...
    if receipt.get("schema_version") != "cisco-acceptance-dispatch-receipt/1":
    ...
    for value, label in ((source_sha, "expected_source_sha"), (observed_sha, "observed_source_sha"), (expected, "expected source")):
    ...
    if receipt.get("dispatch_status") != "dispatched" or receipt.get("http_status") != 204:
        raise CiscoAcceptanceDispatchReceiptError("dispatch receipt does not prove successful API dispatch")

    normalized = {
        "schema_version": "cisco-acceptance-dispatch-receipt-verification/1",
