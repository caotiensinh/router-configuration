...
class VendorTestPlan:
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        payload = {
            ...
            "hardware_certification_possible": all(
                item.disposition is ScenarioDisposition.RUN
                for item in self.scenarios
...
def plan_vendor_tests(
    ...
    for scenario in _normalize_requested(requested):
        ...
        if requirement.requires_fault_injection:
            if not backend.fault_injection_allowed:
                reasons.append("fault injection is not enabled for this backend")
            if not backend.lab_disposable:
                reasons.append("fault injection requires a declared lab/disposable target")
