...
def render_routeros_qos(*, ir: Mapping[str, Any], state: Mapping[str, Any]) -> RouterOSQoSPlan:
    ...
    if missing:
        raise RouterOSQoSRenderError(
            "QoS runtime facts remain deferred; missing: " + ", ".join(missing)
        )
    ...
    for wan_name in sorted(wan_interfaces):
        ...
        default_class: Mapping[str, Any] | None = None
                    ...
                    commands.append(
                        _ensure_mangle(
                            ...
                            fields=(
                                ...
                                "packet-mark=no-mark",
                                "action=mark-packet",
                                f"new-packet-mark={_quote(packet_mark)}",
                                "passthrough=no",
                                "disabled=no",
                            ),
                        )
            ...
            commands.append(
                _ensure_tree(
                    ...
                    fields=(
                        ...
                        f"packet-mark={_quote(packet_mark)}",
        ...
        if default_class is None:
            raise RouterOSQoSRenderError("QoS renderer requires exactly one default class")
        default_name = _identifier(default_class.get("name"), "default QoS class name")
        default_mark = _identifier(
            f"routercfg-qos-{wan_name}-{default_name}",
            "default QoS packet mark",
        )
        commands.append(
            _ensure_mangle(
                command_id=f"qos.19.mark.{wan_name}.default",
                comment=f"routercfg:managed:qos:mark:{wan_name}:default",
                fields=(
                    "chain=forward",
                    f"out-interface={_quote(interface)}",
                    "packet-mark=no-mark",
                    "action=mark-packet",
                    f"new-packet-mark={_quote(default_mark)}",
                    "passthrough=no",
                    "disabled=no",
                ),
            )
