...
STRATEGY = "global_sibling_fq_codel_unmarked_default_marked_priority"
LATENCY_DSCP = 46
LATENCY_PRIORITY = 1
DEFAULT_PRIORITY = 8
RESERVE_PERCENT = 10
...
def _extract_policy_and_targets(
    ...
    if len(qos_matches) != 1:
        raise RouterOSQoSRenderError(
            "QoS renderer requires exactly one qos.policy traffic_policy operation"
        )
    wan_roles = [
        item
        for item in operations
    ...
    if not wan_roles:
        raise RouterOSQoSRenderError(
            "QoS renderer requires at least one compiled WAN topology role"
        )
...
def render_routeros_qos(*, ir: Mapping[str, Any]) -> RouterOSQoSRenderPlan:
    ...
    for index, role in enumerate(wan_roles):
        ...
        interface = _safe_identifier(
            role_attributes.get("interface"), f"WAN role {index}.interface"
        )
        capacity = _positive_int(
            role_attributes.get("capacity_mbps"), f"WAN role {index}.capacity_mbps"
        )
    ...
    commands: list[RouterOSQoSCommand] = [
        RouterOSQoSCommand(
            ...
            section="queue_type",
            command=(
                f":if ([:len [/queue/type/find where name={queue_name_q}]] = 0) "
                f"do={{/queue/type/add name={queue_name_q} kind=fq-codel}}"
            ),
            risk=risk,
        )
    ]
    ...
    for index, target in enumerate(targets, start=1):
        ...
        commands.append(
            RouterOSQoSCommand(
                command_id=f"qos.{index:02d}.default-unmarked",
                operation_id=QOS_OPERATION_ID,
                section="queue_tree",
                command=(
                    f":local rid [/queue/tree/find where name={default_q}]; "
                    f":if ([:len $rid] = 0) do={{/queue/tree/add name={default_q} "
                    f"parent={interface_q} packet-mark=no-mark queue={queue_name_q} "
                    f"priority={DEFAULT_PRIORITY} max-limit={max_limit} disabled=no}} "
                    f"else={{/queue/tree/set $rid parent={interface_q} packet-mark=no-mark "
                    f"queue={queue_name_q} priority={DEFAULT_PRIORITY} max-limit={max_limit} "
                    f"disabled=no}}"
                ),
                risk=risk,
            )
        ...
        commands.append(
            RouterOSQoSCommand(
                ...
                command=(
                    ...
                    f":if ([:len $rid] = 0) do={{/queue/tree/add name={priority_q} "
                    f"parent={interface_q} packet-mark={mark_q} queue={queue_name_q} "
                    f"priority={LATENCY_PRIORITY} limit-at={reserve} max-limit={max_limit} "
                    f"disabled=no}} else={{/queue/tree/set $rid parent={interface_q} "
