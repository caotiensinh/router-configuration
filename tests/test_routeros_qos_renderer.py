...
class RouterOSQoSRendererTests(unittest.TestCase):
    def test_reference_policy_renders_parent_default_fq_codel_for_both_wans(self):
        ...
        self.assertEqual(
            plan["strategy"],
            "parent_fq_codel_default_only_marked_priority_child",
        )
        self.assertEqual(plan["queue_type"], {"name": "routercfg-qos-fq", "kind": "fq-codel"})
        self.assertEqual(
            plan["policy_contract"],
            {
                "latency_dscp": 46,
                "priority": 1,
                "reserve_percent": 10,
        ...
        self.assertIn("priority=1", command_text)
