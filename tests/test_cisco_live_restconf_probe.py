...
def environment(**overrides: str) -> dict[str, str]:
    values = {
        ...
        "CISCO_RESTCONF_USERNAME": "sensitive-user-73x",
        "CISCO_RESTCONF_PASSWORD": "never-persist-this",
...
class CiscoLiveRestconfProbeTests(unittest.TestCase):
    def test_missing_credentials_are_sanitized(self) -> None:
        ...
        self.assertNotIn("sensitive-user-73x", rendered)
        self.assertFalse(evidence["c04_complete"])

    def test_happy_path_is_live_admitted_and_minimized(self) -> None:
        ...
        self.assertEqual(evidence["request_method_scope"], ["GET"])
    ...
    def test_missing_fields_capability_fails_closed(self) -> None:
        ...
        evidence = run_live_probe(environment(), requester)
    ...
    def test_unexpected_exception_is_sanitized(self) -> None:
        def requester(*args, **kwargs):
            del args, kwargs
            raise RuntimeError("router.example sensitive-user-73x never-persist-this")
    ...
    def test_invalid_source_sha_blocks_probe_before_network(self) -> None:
        ...
        evidence = run_live_probe(environment(SOURCE_SHA="bad"), requester)
