...
class QoSIntentTests(unittest.TestCase):
    def test_legacy_policy_remains_deferred_without_invented_runtime_facts(self):
        result = normalize_qos_intent({"enabled": True, "policy": "latency_sensitive_first"})
        self.assertEqual(result.attributes, {"enabled": True, "policy": "latency_sensitive_first"})
    ...
    def test_explicit_policy_is_normalized_deterministically(self):
        result = normalize_qos_intent(
            {
                ...
                "classes": [
                    ...
                    {"name": "video", "priority": 3, "bandwidth_percent": 30, "dscp": [34, 36]},
                ],
            }
        )
        ...
        self.assertEqual(result.attributes["egress_limits_mbps"], {"wan10g": 9500, "wan1g": 950})
        self.assertEqual([item["name"] for item in result.attributes["classes"]], ["voice", "video", "default"])
