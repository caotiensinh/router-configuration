...
class CiscoRestconfPromotionTests(unittest.TestCase):
    ...
    def test_redirect_write_or_credentials_violation_is_rejected(self):
        for field, value in (
            ("redirect_following_allowed", True),
            ...
            with self.subTest(field=field):
                item = successful_evidence()
                item[field] = value
                item["evidence_digest_sha256"] = canonical_sha256(
                    {k: v for k, v in item.items() if k != "evidence_digest_sha256"}
                )
