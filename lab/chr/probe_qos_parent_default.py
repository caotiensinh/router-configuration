...
class CHRQoSParentDefaultProbeError(RuntimeError):
    pass


APPLY_FILE = "routercfg-qos-probe-b-apply.rsc"
ROLLBACK_FILE = "routercfg-qos-probe-b-rollback.rsc"
...
COMMENT = "routercfg:managed:qos-probe-b:voice"
...
def _apply_script() -> str:
    return "\n".join(
        (
            ...
            f"/queue/tree/add name={PARENT} parent=ether2 queue={QUEUE_TYPE} max-limit=100M disabled=no",
...
def _one(rows: list[Mapping[str, Any]], *, label: str, predicate) -> Mapping[str, Any]:
    ...
    if len(matches) != 1:
        raise CHRQoSParentDefaultProbeError(f"expected exactly one {label}, observed {len(matches)}")
    return matches[0]
...
def probe(*, admin_url: str) -> dict[str, Any]:
    ...
    if not {"ether1", "ether2"}.issubset(interfaces):
        raise CHRQoSParentDefaultProbeError("QoS probe requires disposable CHR ether1 management and ether2 test WAN")
    ...
    try:
        ...
        if invalid or disabled:
            raise CHRQoSParentDefaultProbeError(
                f"QoS parent-default formulation is not runtime-valid: invalid={invalid} disabled={disabled}"
            )
        if str(parent.get("queue") or "") != QUEUE_TYPE:
            raise CHRQoSParentDefaultProbeError("parent queue did not retain FQ-CoDel queue type")
        if str(qtype.get("kind") or "").lower() != "fq-codel":
            raise CHRQoSParentDefaultProbeError("managed queue type is not fq-codel")
        managed_default_mangle = [
            row for row in _records(admin, "ip/firewall/mangle")
            if str(row.get("comment") or "").startswith("routercfg:managed:qos-probe-b:default")
        ]
        if managed_default_mangle:
            raise CHRQoSParentDefaultProbeError("parent-default probe unexpectedly created a default mangle rule")

        mutated_sha = base._canonical_digest(_snapshot(admin))
        if mutated_sha == baseline_sha:
            raise CHRQoSParentDefaultProbeError("QoS probe apply did not change configuration digest")
        ...
        if rollback_sha != baseline_sha:
            raise CHRQoSParentDefaultProbeError("QoS probe rollback did not restore exact baseline digest")
    finally:
        for name in TEMP_FILES:
            base._delete_file_if_present(admin, name)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Probe RouterOS parent-default QoS formulation on disposable CHR")
    parser.add_argument("--admin-url", default="http://127.0.0.1:9681")
