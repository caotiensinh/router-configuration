...
def _queue_tree_stats(admin: base.LoopbackCHRAdmin) -> list[Mapping[str, Any]]:
    # RouterOS documents queue counters through `/queue tree print stats`.
    # REST GET maps to a plain `print`, which is sufficient for configuration
    # fields but does not prove that runtime statistics were requested. Use the
    # universal POST command form so the acceptance gate measures `print stats`
    # explicitly rather than inferring queue traversal from config rows.
    _, payload = admin.request("POST", "queue/tree/print", {"stats": ""})
    rows = list(base._rows(payload))
    if not rows:
        raise legacy.CHRQoSPacketFlowError("RouterOS queue tree print stats returned no rows")
    return rows
...
def counters(*, admin_url: str, prepare_payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    def values(row: Mapping[str, Any], label: str) -> dict[str, int]:
        return {
            "packets": legacy._int_counter(row, "packets", label),
            "bytes": legacy._int_counter(row, "bytes", label),
        }
    ...
    return {
        ...
        "runtime": {
            ...
            "default_mangle_count": sum(
                1
                for row in legacy._records(admin, "ip/firewall/mangle")
...
def evaluate(*, prepare_payload: Mapping[str, Any], before: Mapping[str, Any], after_default: Mapping[str, Any], after_ef: Mapping[str, Any], default_flow: Mapping[str, Any], ef_flow: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if any(snapshot.get("counter_source") != "queue_tree_print_stats" for snapshot in (before, after_default, after_ef)):
        raise legacy.CHRQoSPacketFlowError("queue traversal evidence was not collected with queue tree print stats")
    ...
    return {
        "ok": True,
        "acceptance": "PASS",
        "scope": "qos_default_and_ef_leaf_classification_and_queue_traversal",
        "counter_source": "queue_tree_print_stats",
        "classification": {
            "default_dscp": 0,
            "latency_dscp": 46,
            ...
            "default_leaf_packet_delta": d_d - b_d,
            "default_priority_child_delta": d_e - b_e,
            "ef_mangle_packet_delta": e_m - d_m,
            "ef_parent_packet_delta": e_p - d_p,
            "ef_default_leaf_delta": e_d - d_d,
            "ef_priority_child_delta": e_e - d_e,
...
def main() -> int:
    ...
    a.add_argument("--admin-url", required=True)
    a.add_argument("--output", required=True)
    a = sub.add_parser("counters")
    a.add_argument("--admin-url", required=True)
    a.add_argument("--prepare", required=True)
    a.add_argument("--output", required=True)
    a = sub.add_parser("evaluate")
    for flag in ("prepare", "before", "after-default", "after-ef", "default-flow", "ef-flow"):
    ...
    if args.command == "prepare":
        ...
        result = counters(admin_url=args.admin_url, prepare_payload=_read(args.prepare))
